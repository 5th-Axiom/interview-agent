...
- 面试结论卡片 PC 固定 520px、H5 固定 480px；标题与重新生成按钮固定在卡片内，正文内部滚动。优先展示一句结论和最多 5 条关键依据，引用、设置与历史默认折叠。
...
- 开启字幕后缩小圆球，双向文字在固定高度区域内滚动；「我 / AI 面试官」明确区分身份，识别中、打断与待确认用文字标记。向上翻阅暂停自动跟随，「回到最新」恢复；通话控制在页面滚动时保持可达。
- 在 PC / H5、浅色 / 深色与键盘路径检查新增界面；测试模式继续明确标识。
- 不要在业务页面建立第二套颜色、字体或断点系统。
- 不要用圆球动画或颜色代替状态文字，也不要把演示输入当成真实语音能力。
- 面试评估直接展示岗位匹配结论与下一轮建议，并保留原文依据和独立人工复核；不要自动据此改变资格或录用状态，也不要把体验评分表现为候选人成绩。
