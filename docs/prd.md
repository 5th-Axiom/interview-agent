...
- 候选人插话立即收回 AI 播放；已经播放完成的问题不因随后噪声自动重问。纠正、澄清和拒绝展开交由 Prompt 理解，不按回答字数一律丢弃。
- 静音停止实际采音；保存积压时明确暂停，点击继续先补传后恢复。换岗和站内切换保留待确认数据；关闭页面前提醒未保存内容。
- 招聘入口贯穿恢复、反馈与重面；跳过反馈不向招聘方提供放弃的反馈语音。人工编辑在保存过程中继续输入的新内容保留。
- 实现及未完成的真人/性能验收见 [整体改造报告](remediation-report.md)。
...
<tr>
<td>面试结论</td>
<td>AI 根据岗位要求与对话给出面试结论、下一轮建议和关键依据，供招聘方复核</td>
</tr>
<tr>
<td>体验反馈</td>
...
<tr>
<td>评估 Prompt</td>
<td>招聘方写给 AI 的评估要求；在记录详情编辑，只影响本次<strong>评估</strong>的重新生成。</td>
</tr>
<tr>
<td>再次面试申请</td>
...
<tr>
<td>面试结论</td>
<td>先看面试结论和关键依据，按需编辑评估 Prompt</td>
</tr>
<tr>
<td>人工复核</td>
<td>保存招聘方的判断</td>
...
<tr>
<td rowspan="3">面试结论</td>
<td>评估说明：先展示面试结论（建议进入下一轮、建议补充面试、不建议进入下一轮或信息不足），用一句话概括主要理由，再列 3～5 条关键依据，证据少可更少。结论最多 80 字符，每条依据最多 60 字符；未问到、未听清不等于能力不合格。原文引用、设置与历史默认折叠。评估模块 PC 固定 520px、H5 固定 480px，长内容内部滚动，重新生成按钮固定在模块底部。<br>编辑评估 Prompt：点击打开本次评估说明，可修改考察维度、判断依据和输出要求；不可为空，取消则放弃修改。<br>保存 Prompt：仅保存本次评估设置，不改原始对话或其他面试；失败保留输入。点击“重新生成”并确认后，按新 Prompt 生成本次面试结论，人工复核意见保留；生成失败保留旧版本。</td>
</tr>
<tr>
...
<td>复核意见：填写招聘方自己的判断；与 AI <strong>评估</strong>分开保存。</td>
</tr>
<tr>
