...
面试记录列表增加「再次面试」筛选，可选择「全部重面情况」「重面待审核」「可再次面试」，与手机号、岗位、面试状态和日期组合查找。可再次面试指当前招聘入口开放、该记录是候选人在该入口的最新正式面试且已结束，申请已通过、资格尚未使用；待审核和已经使用的资格不计入。列表显示对应标记，通过审核或消费资格后随列表自动刷新更新。

切换筛选回到第一页，进入详情再返回保留筛选；清空同时重置再次面试条件。所有筛选先在服务端执行，再分页返回，本组织以外的记录和试聊不计入。

### 2026-09-15 双向实时字幕

试聊与正式面试共用双向字幕，默认隐藏。开启后在圆球下方展示「AI 面试官 / 我」，按发言顺序排列；圆球缩小，字幕区限定高度并独立滚动，通话按钮在滚动时保持可达。字幕默认跟随最新内容，向上翻阅后停止自动跟随，可点击「回到最新」。

候选人边说边显示「正在转写」，同一语音片段的识别修订原地更新，最终识别结果去掉临时标记。AI 对应句子在开始播放时显示，同一句的多段音频不重复添加；打断标记最后显示的句子，不展示尚未开始播放的句子。暂停或断线保留已有内容，未完成识别标记「转写待确认」。最终识别不等于保存成功，录音积压仍由独立保存提示负责。

关闭字幕不影响采音、播放或保存；再次打开显示最新交流。实时窗口保留本页最近 200 个语音片段，页面刷新后重新开始积累，不用该窗口代替完整原始记录。试聊结束继续立即展示既有的试聊记录。

### 2026-09-15 帮我写面试说明

岗位编辑点击「帮我写面试说明」打开弹窗，表单与草稿预览在弹窗内滚动，不撑开原有编辑页面。关闭弹窗会取消正在进行的生成，保留本页已填写的能力、补充要求与草稿，再次打开可继续编辑。招聘方可从岗位简介提取能力建议，也可直接填写 1–6 项能力，逐项确认「重点 / 常规」，选择开放访谈、按主题推进、固定问题或场景推演，补充必问题和追问要求，再生成面试说明草稿。重点表示优先了解与追问，不是候选人评分权重。

草稿先预览、可修改，点击「填入面试说明」才进入原有 Prompt 编辑框；已有内容时确认替换。岗位名称和简介保留，不自动保存或发布。修改岗位信息或考察设置后，旧草稿提示重新生成；失败或取消保留原有内容，重新生成已人工修改的草稿前确认。辅助设置只服务于本次写作，最终编辑后的 Prompt 仍是唯一面试行为配置，不增加结构化面试计划或运行时流程。

### 2026-09-15 试聊结束记录补充

岗位试聊结束后，原试聊窗口立即切换为本次对话记录，按顺序区分「我」与「AI 面试官」。已保存的文字先展示，不等待录音整理或后台补转写；有补转写时自动更新。AI 生成但未确认播放、部分播放的内容明确标注。记录读取失败保留已展示内容并可重试，没有文字时显示空状态。

返回编辑后，本次岗位编辑页面可通过「查看上次试聊记录」重新打开最近一次记录；重新试聊使用当前 Prompt，创建独立会话，不混入上次对话。此入口只保留当前编辑页面最近一次试聊，刷新或离开页面后不恢复入口。试聊不进入正式面试记录和评估。

### 2026-09-15 Prompt 模板补充

招聘工作台提供「Prompt 模板」标签页，按「固定问题、主题引导、开放访谈」三类组织 5 份完整 Prompt。固定问题包含前端开发题单；主题引导包含后端开发、软件测试场景推演、通用岗位简短初筛；开放访谈包含前端项目交流。用户按面试方式筛选，直接阅读或复制模板内容，也可用模板创建岗位。

固定问题明确主问题、顺序和允许追问的范围；主题引导明确考察范围，模型根据回答调整问题与讨论顺序，跳过已覆盖内容；开放访谈从候选人经历自然展开，用目标与追问边界收束。三类只是模板目录分类，提问、追问与收尾仍写在各自 Prompt 中，共用现有面试引擎。

依据 2026-09-15 最新反馈，删除独立的结构化面试计划、题目数量与建议时长展示，不从 Prompt 另行提取或维护阶段配置。明确计划可以写在 Prompt 文本中，开放访谈可以完全不设阶段；Prompt 是面试行为的唯一配置。

岗位编辑中的「选择模板」可阅读并应用完整文本；已有 Prompt 时先确认替换，已填写的岗位名称和简介保留。选用后是可自由编辑的岗位草稿，不自动保存或发布，可立即使用当前内容试聊。模板更新不回写已创建岗位或已冻结的面试快照。
...
<tr>
<td>字幕</td>
<td>AI 与候选人的实时对话文字，可以显示或隐藏，临时识别会原地修订；完整记录另行保存</td>
</tr>
<tr>
<td>面试记录</td>
...
<tr>
<td>实时字幕：开启后按顺序显示 AI 当前话语与候选人的识别结果，回答时保留已有交流；字幕默认<strong>隐藏</strong>。临时识别标记「正在转写」，最终结果原地更新。</td>
</tr>
<tr>
...
<tr>
<td>选择模板：按面试方式浏览内置 Prompt 的完整文本；浏览不改变当前内容。</td>
</tr>
<tr>
<td>使用此模板：把完整 Prompt 填入编辑框；已有内容时先确认替换，保留已填写的岗位名称与简介。</td>
</tr>
<tr>
