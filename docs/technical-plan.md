...
前端请求放 `features/admin/api.ts`，状态与取消逻辑放 `use-prompt-assistant.ts`。请求代次与输入快照阻止过期结果覆盖当前内容；收起助手会取消请求并保留本页输入。能力与面试方式仅用于写作，不进入会话运行时；仅用户确认填入的 Prompt 参与已有保存、发布和试聊快照。
