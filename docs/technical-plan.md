...
`GET /api/records` 增加 `retry` 枚举参数（空值、`pending`、`available`）。查询从 `retry_requests` 派生 `pending` 和 `retry_available`，后者要求已批准且 `used_by IS NULL`、正式场次已结束、入口开放且同候选人同入口没有更新的正式场次。与既有条件一起在 SQL 分页前过滤，不新增或修改重面资格，不改变审核和一次性消费事务。列表沿用组织鉴权与正式场次限制。

`useRecordFilters` 持有再次面试条件，沿用会话级筛选保存；兼容旧缓存缺少该字段，新增或修改条件时重置页码。列表每 5 秒刷新现有查询，审批或资格消费后同步更新结果。

### 2026-09-15 双向字幕实现

客户端在 Relay 初始化时声明 `captions: true`，旧客户端默认关闭，避免新增 `user_caption` 事件破坏已有协议解析。`LiveASR` 为供应商片段分配独立字幕 ID 与递增修订号，通过独立回调即时推送临时/最终结果；同一供应商连接内原地修订，连接轮换后片段 ID 隔离，退休连接仅更新自己的片段，关闭后的回调失效。该回调不进入保存队列、不触发模型生成，原有静音归并、保存重试和连续上下文保持原逻辑。开发测试输入在保存确认后发送最终字幕。

`use-voice-session` 沿用连接代次和 epoch 校验接收字幕，AI 字幕只在当前回复音频的实际播放开始回调中加入，以 event ID 去除同一句多音频块的重复。纯 reducer 保留发言顺序、丢弃旧修订、标记最后显示的被打断句子，以及断线后的未确认转写，窗口限制为最近 200 个片段。只影响展示，不修改服务端证据或 Prompt。

`LiveCaptions` 提供固定高度的内部滚动区：默认跟随底部，用户向上滚动后保留阅读位置，点击「回到最新」恢复跟随；隐藏字幕只卸载展示，Hook 中的数据继续更新。字幕区可键盘聚焦和滚动，临时文字不通过 live region 重复朗读，避免与面试语音叠加。

### 2026-09-15 试聊结束记录实现

`GET /api/sessions/:id/preview-history` 验证招聘方身份、组织访问范围与试聊模式，直接读取持久化事件、播放确认和补转写任务状态。复用连续对话投影，ASR 修订保留原发言顺序，只展示当前修订；AI 全文与播放状态分别返回。不返回音频存储地址、模型配置或正式面试评估，不增加数据库表。

试聊仍先停止采音与播放、补传待保存内容，再从语音窗口切换到记录。结束回调触发立即刷新，不等待 Worker；补转写尚在排队或运行时每 2 秒刷新，完成后停止轮询，失败保留原内容与重试入口。当前编辑页面可重新打开最近一次记录；新试聊按 session ID 隔离缓存。语音结束通知丢失时，服务端快照同样触发结束流程；正在补传时阻止重新连接，组件卸载后旧完成回调不能修改新页面。

### 2026-09-15 面试说明辅助写作实现

`POST /api/admin/prompt-assistant` 接收 `goals`（由岗位简介提取能力）或 `draft`（由确认后的能力、优先级、方式与补充要求生成 Prompt）请求。接口验证招聘方身份、来源与输入长度，按组织限流 12 次/分钟。复用 `structured` 的 assessment 模型配置与并发预算，60 秒总超时，支持请求取消；模型调用不占用数据库事务，不创建岗位、版本、会话或评估任务，不自动重试生成。返回内容经 schema 校验，测试替身明确返回 `testMode`。

前端请求放 `features/admin/api.ts`，状态与取消逻辑放 `use-prompt-assistant.ts`。辅助表单复用原生模态 `Dialog`，脱离页面文档流，标题与关闭按钮保留在顶部，表单和草稿在弹窗内部滚动；替换与重新生成确认使用独立确认弹窗。请求代次与输入快照阻止过期结果覆盖当前内容；关闭助手弹窗会取消请求并保留本页输入。能力与面试方式仅用于写作，不进入会话运行时；仅用户确认填入的 Prompt 参与已有保存、发布和试聊快照。

### 2026-09-15 Prompt 模板实现

`/admin/templates` 与岗位编辑共用 `features/admin/prompt-templates.ts` 的内置目录。每个模板只包含标识、名称、面试方式、适用说明、默认岗位信息和完整 `prompt` 字符串。面试方式仅用于筛选，不参与运行时分支；没有阶段、时长或问题数组，也没有从这些字段拼接 Prompt 的逻辑。展示、复制和创建草稿直接使用同一字符串。模板目录随代码发布，本期不增加自定义模板数据库或 CRUD 接口。

目录分类统一为「固定问题、主题引导、开放访谈」，共 5 份模板。软件测试场景推演和通用岗位简短初筛归入主题引导；原后端分段模板改为按主题探索，Prompt 允许根据已有信息调整讨论顺序。原模板 ID 与链接保留，模板页和岗位选择器使用同一分类与文本，不新增运行模式或改写已有岗位。

模板页通过 `/admin/roles/new?template=<id>` 初始化新岗位草稿；编辑器内原地选择模板并确认覆盖已有 Prompt，不跳转丢失输入。有效模板只在新岗位首次初始化时读取，未知模板明确提示；已有岗位从保存内容初始化。模板应用沿用现有草稿校验、并发版本保护、发布快照与试聊快照。题序和面试计划由 Prompt 驱动，不新增按题号推进或清空上下文的状态机。

评估输出使用版本 2 结构：`conclusion {verdict, summary, sources}` 与最多 5 条 `items`。结论限 80 字符，依据每条限 60 字符；结合冻结的岗位要求，按预算分批读取完整原始对话并更新有来源的整体结论。校验来源存在，非“信息不足”的结论必须引用有效候选人回答；无有效回答直接返回信息不足。只迁移原默认评估要求，保留自定义要求与历史结果；新结论不自动改变面试资格或录用状态。详情的评估卡片固定高度（PC 520px / H5 480px），结论置顶，内容独立滚动，设置、引用与历史按需展开。
...
| 根据评估 Prompt 给出面试结论、下一轮建议和关键依据 | 校验引用是否真实存在；保留版本，人工复核独立保存。 |
