  ...
  return (
    ...
    >
      <div className="row between">
        <h3 id="prompt-assistant-title">帮我写面试说明</h3>
        <Button
          variant="quiet icon"
          aria-label="收起面试说明助手"
          onClick={onClose}
      ...
      {!name.trim() && <Notice>请先填写上方的岗位名称。</Notice>}
        ...
        {a.goals.map((goal, index) => (
          <div className="prompt-goal-row" key={index}>
            ...
            <Button
              ...
              onClick={() => a.setGoals(a.goals.filter((_, i) => i !== index))}
            >
              <X />
            </Button>
          </div>
      ...
      {a.draft && (
        <div className="prompt-assistant-result">
          ...
          <div className="row between">
            <span className="small muted">填入后仍可修改、试聊和保存。</span>
            <Button
              variant="primary"
              disabled={!canApply}
